...
links), reporting best-practice notes as non-fatal warnings. It SKIPs when `stac-check` isn't
installed, so local runs stay zero-setup; CI (`.github/workflows/ci.yml`) installs it and runs
all five, so invalid STAC fails the build on push/PR.
