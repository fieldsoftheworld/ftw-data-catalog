# FTW Global — Sentinel-2 Planting & Harvest Composites (2024)
...
ds = xr.open_zarr("https://data.source.coop/ftw/global-data/features/zarr/alpha/global.zarr").pipe(rasterix.assign_index)  # global mosaic
```

Per-tile COGs are indexed by the STAC-GeoParquet item index (`https://data.source.coop/ftw/global-data/features/2024/items.parquet`); each item has a `planting` and a `harvest` 5-band COG (B02/B03/B04/B08/N_VALID_PIXELS) in its native UTM zone.
